...
function generateProductData(type, count) {
  const data = [];
  const supplier =
    type === "GENUINE" ? randomChoice(["JLR Official", "OEM Parts Co"]) : "Aftermarket Supplies";
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const band2 = randomFloat(retailPrice * 0.9, retailPrice * 0.9);
    const band3 = randomFloat(retailPrice * 0.85, retailPrice * 0.85);
    const band4 = randomFloat(retailPrice * 0.8, retailPrice * 0.8);
