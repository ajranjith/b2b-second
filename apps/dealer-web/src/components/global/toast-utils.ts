...
export const commonToasts = {
  ...
  notFound: (itemType: string) => showToast.error("Not Found", `${itemType} not found`),

  unauthorized: () => showToast.error("Unauthorized", "Please log in to continue"),

  validationError: (message: string) => showToast.error("Validation Error", message),

  // Warnings
  unsavedChanges: () =>
    showToast.warning("Unsaved Changes", "You have unsaved changes that will be lost"),
