...
export function DealerHeaderBar() {
  ...
  const [slides, setSlides] = useState<HeroBannerItem[]>([
    { id: "dealer-default-1", mediaUrl: "/brand/hotbray/jlr-dual-brand.webp" },
    { id: "dealer-default-2", mediaUrl: "/brand/hotbray/pexels-jan-kopiva-3399938.webp" },
    { id: "dealer-default-3", mediaUrl: "/brand/hotbray/pexels-pixabay-162553.webp" },
    { id: "dealer-default-4", mediaUrl: "/brand/hotbray/zoom-air-suspension.webp" },
  ]);
      ...
      .then((data: any[]) => {
        if (data.length) {
          const mapped = data.map((slide, index) => ({
            id: slide.id ?? slide.src ?? slide.url ?? `dealer-banner-${index}`,
