...
export class DealerImportService extends ImportService<DealerAccountRow> {
  ...
  validateRow(row: DealerAccountRow, rowNumber: number): ValidationResult {
    ...
    const genuineTierError = this.validateEnum(row['Genuine Tier'], 'Genuine Tier', this.VALID_TIERS);
    if (genuineTierError) errors.push(genuineTierError);

    const esTierError = this.validateEnum(row['Aftermarket ES Tier'], 'Aftermarket ES Tier', this.VALID_TIERS);
    if (esTierError) errors.push(esTierError);

    const bTierError = this.validateEnum(row['Aftermarket B Tier'], 'Aftermarket B Tier', this.VALID_TIERS);
  ...
  parseRow(row: DealerAccountRow, batchId: string, rowNumber: number): ParsedDealerRow {
    ...
    const accountNumber = this.trimString(row['Account Number']);
    ...
    return {
      ...
      defaultShippingMethod: this.trimString(row['Default shipping method']),
      notes: this.trimString(row['Notes']),
      genuineTier: this.trimString(row['Genuine Tier']),
      aftermarketEsTier: this.trimString(row['Aftermarket ES Tier']),
      aftermarketBTier: this.trimString(row['Aftermarket B Tier']),
      tempPassword: this.trimString(row['Temp password']),
    ...
    for (const row of validRows) {
      await this.prisma.$transaction(async (tx) => {
        ...
        const dealerAccount = await tx.dealerAccount.upsert({
          where: { accountNo: row.accountNumber! },
          update: {
        ...
        const tempPassword = row.tempPassword || this.generateTempPassword();
        ...
        // 4. UPSERT DealerBandAssignment (3 required assignments)
        const tierAssignments = [
          { partType: PartType.GENUINE, tierCode: row.genuineTier! },
          { partType: PartType.AFTERMARKET, tierCode: row.aftermarketEsTier! },
          { partType: PartType.BRANDED, tierCode: row.aftermarketBTier! }
        ];

        for (const assignment of tierAssignments) {
          await tx.dealerBandAssignment.upsert({
            where: {
