...
export class OrderEngine {
  async createOrder(input: OrderInput) {
    console.log(
      `📦 [Order Engine] Processing order from ${input.source} for dealer ${input.dealerAccountId}`,
    );
    ...
    for (const item of input.items) {
      ...
      if (product.stock && product.stock.freeStock < item.qty) {
        console.warn(
          `⚠️ [Order Engine] Short stock for ${item.productCode}: requested ${item.qty}, only ${product.stock.freeStock} available.`,
        );
      }
