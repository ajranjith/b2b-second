 ...
 * 
 * This page is a React Server Component that queries the data store DIRECTLY.
 * No API routes, no client-side fetching, no network hops.
 * 
 * Data Flow:
 * 1. Next.js receives request
 * 2. Server Component queries data store via service API
...
// --- SERVER COMPONENT: FETCHES DATA FROM data store ---
async function OrderList({
    ...
    if (query) {
        ...
        filtered = filtered.filter((order) =>
            ...
            order.poRef?.toLowerCase().includes(q) ||
            order.dealerAccount?.companyName?.toLowerCase().includes(q) ||
            order.dealerAccount?.accountNo?.toLowerCase().includes(q)
        );
    }
    if (status) {
    ...
    const orders = filtered.slice(skip, skip + pageSize);
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className={`px-6 ${cellPadding} text-left text-xs font-semibold text-gray-500 uppercase tracking-wider`}>
                                Order & PO
                            </th>
                            <th className={`px-6 ${cellPadding} text-left text-xs font-semibold text-gray-500 uppercase tracking-wider`}>
                                Dealer
                            ...
                            <th className={`px-6 ${cellPadding} text-right text-xs font-semibold text-gray-500 uppercase tracking-wider`}>
                                Total
                            </th>
                            <th className={`px-6 ${cellPadding} text-right text-xs font-semibold text-gray-500 uppercase tracking-wider`}>
                                Actions
                            ...
                            <tr key={order.id} className="hover:bg-gray-50 transition-colors group">
                                ...
                                <td className={`px-6 ${cellPadding} whitespace-nowrap text-right`}>
                                    <span className="text-sm font-bold text-gray-900">
                                        GBP {Number(order.total).toFixed(2)}
                                    </span>
                                </td>
...
export default async function AdminOrdersPage({
    ...
    const density = (params?.density === 'dense' ? 'dense' : 'comfortable') as 'comfortable' | 'dense';
    ...
    return (
        <div className="space-y-6">
            ...
            <Suspense key={`${page}-${query}-${status}-${density}`} fallback={<OrderListSkeleton />}>
                <OrderList page={page} query={query} status={status} density={density} />
            </Suspense>
        </div>
    );
}
