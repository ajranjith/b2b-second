...
export default function DealersPage() {
  ...
  const handleResetPassword = async (dealerId: string, email: string) => {
    try {
      ...
      const response = await api.post(`/admin/dealers/${dealerId}/reset-password`);
      const tempPassword = response.data?.tempPassword;
      if (tempPassword) {
        await navigator.clipboard.writeText(tempPassword);
        toast.success(`Password reset. Copied new temp password for ${email}.`);
      } else {
  ...
  const columns: ColumnDef<Dealer>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        const user = row.original.users[0];
        return user ? (
          <div className="text-sm text-slate-600">{user.user.email}</div>
        ) : (
          <div className="text-sm text-slate-400">-</div>
        );
      },
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const email = dealer.users[0]?.user.email || "";
                    ...
                    <TableCell colSpan={columns.length} className="h-32 text-center">
                      <div className="flex flex-col items-center justify-center gap-2">
                        ...
                        <span className="font-medium text-slate-700">
                          {searchQuery || statusFilter !== "ALL" ? "No dealers match your filters" : "No dealers yet"}
                        </span>
                        <span className="text-sm text-slate-500">
                          {searchQuery || statusFilter !== "ALL"
                            ? "Try adjusting your search or filter criteria."
                            : "Click \"Create Dealer\" to add your first dealer account."}
                        </span>
                      </div>
                    </TableCell>
                  </TableRow>
