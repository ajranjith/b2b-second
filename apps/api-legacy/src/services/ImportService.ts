...
type SpecialPriceMeta = { newsArticleId?: string | null; source?: string | null };
...
export class ImportService {
  ...
  async processImport(
    ...
    switch (importType) {
      ...
      case ImportType.SPECIAL_PRICES:
        ...
        return this.importSpecialPrices(filePath, batchId, specialPriceWindow, specialPriceMeta);
  ...
  private async importDealers(filePath: string, batchId: string) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const shippingMethodRaw = getValue(row, [
      ...
      const aftermarketEsTier = aftermarketEsTierRaw
        ? String(aftermarketEsTierRaw).trim().toUpperCase()
        : "";
      const aftermarketBrTier = aftermarketBrTierRaw
        ? String(aftermarketBrTierRaw).trim().toUpperCase()
        : "";
  ...
  private async importSupersessions(filePath: string, batchId: string) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const originalPartCode = originalPartCodeRaw
        ? String(originalPartCodeRaw).trim().toUpperCase()
        : "";
      const replacementPartCode = replacementPartCodeRaw
        ? String(replacementPartCodeRaw).trim().toUpperCase()
        : "";
  ...
  private async importSpecialPrices(
    filePath: string,
    batchId: string,
    window: SpecialPriceWindow,
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      await this.prisma.specialPrice.upsert({
        ...
        update: {
          ...
          newsArticleId: meta?.newsArticleId ?? null,
  ...
  private async importBackorderUpdates(filePath: string, batchId: string) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const ourOrderNoRaw = getValue(row, [
        "Our Number / ERP Order Number",
        "ERP Order Number",
        "Our Number",
      ]);
