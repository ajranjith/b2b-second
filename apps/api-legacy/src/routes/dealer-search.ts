  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply) => {
      ...
      try {
        ...
        const priceMap = await pricingService.resolvePrices(productIds, user.dealerAccountId);
  ...
  fastify.get(
    ...
    async (request: AuthenticatedRequest, reply) => {
      ...
      try {
        ...
        // Resolve price
        const priceResolution = await pricingService.resolvePrice(product.id, user.dealerAccountId);

        // Resolve prices for equivalents
        const equivalentIds = product.equivalents.map((eq) => eq.equivalentProductId);
        const equivalentPriceMap =
