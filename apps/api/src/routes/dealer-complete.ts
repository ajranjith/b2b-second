...
const dealerCompleteRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.patch<{ Params: { itemId: string } }>('/cart/items/:itemId', {
    preHandler: requireAuth
  }, async (request: AuthenticatedRequest, reply) => {
   ...
   */
  fastify.delete<{ Params: { itemId: string } }>('/cart/items/:itemId', {
    preHandler: requireAuth
  }, async (request: AuthenticatedRequest, reply) => {
    const user = request.user;
  ...
  fastify.get('/backorders/export', {
    ...
    try {
      ...
      reply.header('Content-Type', 'text/csv');
      reply.header('Content-Disposition', `attachment; filename="backorders-export-${Date.now()}.csv"`);
      return reply.send(csv);
    } catch (error: any) {
      fastify.log.error(error);
