...
async function main() {
    ...
    console.log('Latest product batch:', lastProductBatch.id, 'keep products:', keepProductCodes.length);
    ...
    await prisma.$transaction(async (tx) => {
        ...
        if (dealerAccountIdsToDelete.length > 0) {
            ...
            await tx.dealerDiscountTier.deleteMany({ where: { dealerAccountId: { in: dealerAccountIdsToDelete } } });
            await tx.dealerBandAssignment.deleteMany({ where: { dealerAccountId: { in: dealerAccountIdsToDelete } } });
            await tx.dealerUser.deleteMany({ where: { dealerAccountId: { in: dealerAccountIdsToDelete } } });
        }
        ...
        if (dealerAccountNosToDelete.length > 0) {
            ...
            await tx.backorderLineContract.deleteMany({ where: { accountNo: { in: dealerAccountNosToDelete } } });
