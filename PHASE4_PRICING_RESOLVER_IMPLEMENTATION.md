...
class PricingService {
  ...
  async getDealerPricingContext(
    dealerAccountId: string
  ): Promise<DealerPricingContext | null>
  ...
  // Checkout snapshot
  async snapshotPriceForCheckout(
    cartItemId: string,
    dealerAccountId: string
  ): Promise<number>
}
...
expect(price1.price).toBe(100.00);
...
expect(price2.price).toBe(75.00);
...
expect(price3.price).toBe(100.00);
...
expect(cart1.items[0].yourPrice).toBe(50.00);
...
expect(cart2.items[0].yourPrice).toBe(45.00); // ✅ Price updated

// 5. Create order (snapshot price)
const order = await orderService.createOrder(dealerUserId, dealerAccountId, {});
expect(order.lines[0].unitPriceSnapshot).toBe(45.00); // ✅ Snapshotted at checkout
...
expect(historicalOrder.lines[0].unitPriceSnapshot).toBe(45.00); // ✅ Still £45.00
```
