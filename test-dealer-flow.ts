...
class DealerProcessTester {
  ...
  private async testProductSearch() {
    return this.runTest("Product Search - Basic Query", async () => {
      const response = await fetch(`${API_BASE_URL}/dealer/search?q=Product`, {
      ...
      console.log(
        `    - First product: ${data.results[0].productCode} - ${data.results[0].description}`,
      );
  ...
  private async testProductSearchEntitlementFiltering() {
    return this.runTest("Product Search - Entitlement Filtering", async () => {
      const response = await fetch(`${API_BASE_URL}/dealer/search?limit=100`, {
        headers: { Authorization: `Bearer ${this.testData.authToken}` },
      });
      ...
      if (entitlement === "AFTERMARKET_ONLY") {
        ...
        if (hasGenuine) {
          throw new Error(
            "Entitlement filter failed: GENUINE products visible to AFTERMARKET_ONLY dealer",
          );
        }
      }
  ...
  private async testGetProductDetail() {
    return this.runTest("Get Product Detail", async () => {
      ...
      const response = await fetch(`${API_BASE_URL}/dealer/product/${this.testData.productCode}`, {
        headers: { Authorization: `Bearer ${this.testData.authToken}` },
      });
  ...
  private async testGetCart() {
    return this.runTest("Get Cart", async () => {
      const response = await fetch(`${API_BASE_URL}/dealer/cart`, {
        headers: { Authorization: `Bearer ${this.testData.authToken}` },
      });
  ...
  private async testAddItemToCart() {
    return this.runTest("Add Item to Cart", async () => {
      ...
      const response = await fetch(`${API_BASE_URL}/dealer/cart/items`, {
        method: "POST",
        headers: {
  ...
  private async testUpdateCartItem() {
    return this.runTest("Update Cart Item Quantity", async () => {
      const cartResponse = await fetch(`${API_BASE_URL}/dealer/cart`, {
        headers: { Authorization: `Bearer ${this.testData.authToken}` },
      });
      ...
      const updateResponse = await fetch(`${API_BASE_URL}/dealer/cart/items/${itemId}`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.testData.authToken}`,
  ...
  private async testRemoveCartItem() {
    return this.runTest("Remove Item from Cart", async () => {
      ...
      // Add an item
      const addResponse = await fetch(`${API_BASE_URL}/dealer/cart/items`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.testData.authToken}`,
      ...
      // Remove it
      const removeResponse = await fetch(`${API_BASE_URL}/dealer/cart/items/${itemId}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${this.testData.authToken}` },
      });
  ...
  private async testCheckout() {
    return this.runTest("Place Order (Checkout)", async () => {
      ...
      const response = await fetch(`${API_BASE_URL}/dealer/checkout`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${this.testData.authToken}`,
